...
viewof xi     = Inputs.range(minmax(p2,1), {step: s(p2,1),  label: "ξ", value:p2[0][1]})
viewof alpha  = Inputs.range(minmax(p2,2), {step: s(p2,2),  label: "α", value:p2[0][2]})
viewof gamma2 = Inputs.range(minmax(p2,3), {step: s(p2,3),  label: "γ", value:p2[0][3]})
viewof rho2   = Inputs.range(minmax(p2,4), {step: s(p2,4),  label: "ρ", value:p2[0][4]})
viewof eta    = Inputs.range(minmax(p2,5), {step: s(p2,5),  label: "η", value:p2[0][5]})
viewof b2     = Inputs.range(minmax(p2,6), {step: s(p2,6),  label: "b", value:p2[0][6]})
viewof c2     = Inputs.range(minmax(p2,7), {step: s(p2,7),  label: "c", value:p2[0][7]})
viewof mu2    = Inputs.range(minmax(p2,8), {step: s(p2,8),  label: "μ", value:p2[0][8]})

data2 = resdb.query(`
  SELECT timestep::INT as timestep, L::INT as L, value, name
  FROM sourcesink2
  WHERE
  name = '${f(beta2)}_${f(xi)}_${f(alpha)}_${f(gamma2)}_${f(rho2)}_${f(eta)}_${f(b2)}_${f(c2)}_${f(mu2)}'
`)
```
...
viewof delta3 = Inputs.range(minmax(p3,6), {step: s(p3,6),  label: "δ", value:p3[0][6]})

data3 = resdb.query(`
  SELECT timestep::INT as timestep, L::INT as L, value, name
  FROM sourcesink3
  WHERE
  name = '${f(beta3)}_${f(gamma3)}_${f(rho3)}_${f(b3)}_${f(c3)}_${f(mu3)}_${f(delta3)}'
`)
```
...
```{ojs}
//| panel: sidebar
viewof param1 = Inputs.radio(["β", "ρ", 'b'], {label: 'x', value: "β"})
viewof param2 = Inputs.radio(["β", "ρ", 'b'], {label: 'y', value: 'b'})

viewof range0 = Inputs.range(minmax(p3,0), {step: s(p3,0), label: "β", disabled: param1 == "β" || param2 == "β"})
viewof range1 = Inputs.range(minmax(p3,1), {step: s(p3,1), label: "γ", value:p3[0][1], disabled: true})
viewof range2 = Inputs.range(minmax(p3,2), {step: s(p3,2), label: "ρ", disabled: param1 == "ρ" || param2 == "ρ"})
viewof range3 = Inputs.range(minmax(p3,3), {step: s(p3,3), label: "b", disabled: param1 == 'b' || param2 == 'b'})
viewof range4 = Inputs.range(minmax(p3,4), {step: s(p3,4), label: "c", value:p3[0][4], disabled: true})
viewof range5 = Inputs.range(minmax(p3,5), {step: s(p3,5), label: "μ", value:p3[0][5], disabled: true})
```
...
param_lookup = {
    return {"β": 0, 'gamma': 1, "ρ": 2, 'b': 3, 'c': 4, 'm': 5}
}
...
data_hm = {
  ...
  for (let i=0; i < d.length; i++) { 
    const p_split = d[i].name.split('_')

    const hm_vals = {
      'L': d[i].L,
      'delta': p_split[6],
      'param1': parseFloat(p_split[param_lookup[param1]]),
      'param2': parseFloat(p_split[param_lookup[param2]]),
      'value': d[i].value_prop
    }

    // We only look at values specified by ranges that do not vary.
    if (parseFloat(p_split[1]) === range1 && parseFloat(p_split[4]) === range4 && parseFloat(p_split[5]) === range5) {

        if (param1 == "β" && param2 == 'b') {
           if (parseFloat(p_split[2]) == range2) {
             out.push(hm_vals)

           }
        } else if (param1 == "β" && param2 == "ρ") {
           if (parseFloat(p_split[3]) == range3) {           
             out.push(hm_vals)

           }
        } else if (param1 == 'ρ' && param2 == "b") {
           if (parseFloat(p_split[0]) == range0) {
             out.push(hm_vals)
...
PlotDev.plot({
  ...
  x: { label: param1 },
  y: { label: param2 },
  facet: { data: data_hm, y: "L", x: "delta" },
  marks: [
    PlotDev.raster(data_hm, {
      x: "param1",
      y: "param2",
      fill: "value",
      interpolate: "nearest"
