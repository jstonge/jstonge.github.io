...
viewof r2 = Inputs.form({
  x: Inputs.radio(axis_vars, {label: "x", value: ax0}),
  y: Inputs.radio(axis_vars, {label: "y", value: ax1})
})


function get_fixed_params() {
  const all_params = [...Object.keys(p2)]
  const varying_params = new Set(axis_vars.concat(facet_var))
  const diff_params = new Set(all_params.filter((x) => !varying_params.has(x)))
  return Array.from(diff_params)
}

fp2 = get_fixed_params();

// If there is a fx var, we can ignore it. Here alpha.
viewof s2b = Inputs.form({
  ax0: Inputs.range(p2[ax0]['minmax'], {step: p2[ax0]['s'], label: ax0, disabled: r2['x'] == ax0 || r2['y'] == ax0}),
  ax1: Inputs.range(p2[ax1]['minmax'], {step: p2[ax1]['s'], label: ax1, disabled: r2['x'] == ax1 || r2['y'] == ax1}),
  ax2: Inputs.range(p2[ax2]['minmax'], {step: p2[ax2]['s'], label: ax2, disabled: r2['x'] == ax2 || r2['y'] == ax2}),
...
data_hm2 = {
  ...
  for (let i=0; i < data2b.length; i++) { 
    ...
    const ax0_val = parseFloat(p_split[lookup2['name2idx'][ax0]])
    const ax1_val = parseFloat(p_split[lookup2['name2idx'][ax1]])
    const ax2_val = parseFloat(p_split[lookup2['name2idx'][ax2]])
    const fp0_val = parseFloat(p_split[lookup2['name2idx'][fp2[0]]])
    const fp1_val = parseFloat(p_split[lookup2['name2idx'][fp2[1]]])
    const fp2_val = parseFloat(p_split[lookup2['name2idx'][fp2[2]]])
    const fp3_val = parseFloat(p_split[lookup2['name2idx'][fp2[3]]])
    const fp4_val = parseFloat(p_split[lookup2['name2idx'][fp2[4]]])

    const hm_vals = {
      'L': data2b[i].L,
      'fx' : p_split[lookup2['name2idx'][facet_var]], 
    ...
    if (fp0_val === s2b['fp0'] && fp1_val === s2b['fp1'] && fp2_val === s2b['fp2'] && fp3_val == s2b['fp3'] && fp4_val == s2b['fp4'] ) {

        // if ax1 == x && ax2 ==y, then ax 0 == z
        if (r2['x'] == ax1 && r2['y'] == ax2 && ax0_val == s2b['ax0']) {
             out.push(hm_vals)
        } else if (r2['x'] == ax0 && r2['y'] == ax2 && ax1_val == s2b['ax1']) {
             out.push(hm_vals)
        } else if (r2['x'] == ax0 && r2['y'] == ax1 && ax2_val == s2b['ax2']) {
             out.push(hm_vals)
        }
    }
  }
