...
data = resdb.query(`
  ...
  FROM sourcesink1
  WHERE
  name = '${f(s1['ax0'])}_${f(s1['fp0'])}_${f(s1['fp1'])}_${f(s1['ax2'])}_${f(s1['ax1'])}_${f(s1['fp2'])}'
`)
...
pd1 = phase_diagram(data_hm, r1['x'], r1['y'])
...
```{ojs}
//| panel: sidebar
>>>>>>> parent of 2fbcf92 (putting figures side by side)
...
data2 = resdb.query(`
  ...
  FROM sourcesink2
  WHERE
  name = '${f(s2['ax0'])}_${f(s2['fp0'])}_${f(s2['fp1'])}_${f(s2['fp2'])}_${f(s2['ax1'])}_${f(s2['ax2'])}_${f(s2['fp3'])}_${f(s2['fp4'])}_${f(s2['fp5'])}'
`)
...
// index should map to below sliders, e.g. fp0 == γ == "second spot in `name` string" 
viewof s3 = Inputs.form({
  ax0: Inputs.range(p3[ax_vars3[0]]['minmax'], {step: p3[ax_vars3[0]]['s'], label: ax_vars3[0]}),
  ax1: Inputs.range(p3[ax_vars3[1]]['minmax'], {step: p3[ax_vars3[1]]['s'], label: ax_vars3[1]}),
  ax2: Inputs.range(p3[ax_vars3[2]]['minmax'], {step: p3[ax_vars3[2]]['s'], label: ax_vars3[2]}),
  ...
  fp3: Inputs.range(p3[fp3[3]]['minmax'], {step: p3[fp3[3]]['s'], label: fp3[3], value: p3[fp3[3]]['first_val']})
})
...
data3a = resdb.query(`
  ...
  FROM sourcesink3
  WHERE
  name = '${f(s3["ax0"])}_${f(s3["fp0"])}_${f(s3["ax1"])}_${f(s3["ax2"])}_${f(s3["fp1"])}_${f(s3["fp2"])}_${f(s3["fp3"])}'
`)
...
function plot_time_evo(d, y_vals, pal) {
  ...
  return Plot.plot({
    x: {type:"log"},
    marginLeft: 50,
    color: { scheme: pal, type: "ordinal", legend: true },
    marks: [
      Plot.lineY(Array.from(global_mean.values()), {strokeDasharray: "5,3"}),
      Plot.line(
        d, {
